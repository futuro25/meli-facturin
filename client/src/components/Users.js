...
export default function Users() {
  ...
  const usernameValidation = (username) => {
    return data?.find(user => user.username === username) || false;
  }

  const emailValidation = (email) => {
    return data?.find(user => user.email === email) || false;
  }
                    ...
                    <tbody className="bg-white ">
                      {
                        ...
                        dataFiltered.map((user, index) => (
                          ...
                          (
                            <tr>
                              <td colSpan={8} className="border-b border-slate-100  p-4  text-slate-500 ">No data</td>
                            </tr>
                          )
                      }
                    </tbody>
